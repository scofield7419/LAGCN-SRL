...
class BiRecurrentConvBiAffine(nn.Module):
    def __init__(self, word_dim, num_words, char_dim, num_chars, pos_dim, num_pos, num_filters, kernel_size, rnn_mode, hidden_size, num_layers,
    ...
    def decode_mst(self, input_word, input_char, input_pos, mask=None, length=None, hx=None, leading_symbolic=0):
        ...
        return args_parser.decode_MST(energy.data.cpu().numpy(), length, leading_symbolic=leading_symbolic, labeled=True)
...
class StackPtrNet(nn.Module):
    ...
    def loss(self, input_word, input_char, input_pos, heads, stacked_heads, children, siblings, stacked_types, label_smooth,
        ...
        if self.skipConnect:
            output_dec, _, mask_d, _ = self._get_decoder_output_with_skip_connect(output_enc, heads, stacked_heads, siblings, skip_connect, hn,
...
class NewStackPtrNet(nn.Module):
    ...
    def loss(self, input_word, input_lemma, input_char, input_pos, heads, stacked_heads, children, siblings, stacked_types, previous, next,
             label_smooth,
             skip_connect=None, mask_e=None, length_e=None, mask_d=None, length_d=None, hx=None):
        # output from encoder [batch, length_encoder, hidden_size]
        ...
        if self.skipConnect:
            ...
            # print 'SE USA ESTE'
            output_dec, _, mask_d, _ = self._get_decoder_output(output_enc, heads, stacked_heads, siblings, previous, next, hn, mask_d=mask_d,
                                                                length_d=length_d)
